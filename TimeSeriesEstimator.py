...
def time_series_split(X, test_size=.2, number=False, output_numpy=True):
    ...
    if test_size <= 1 and not number:
        ...
        raise ValueError(
            "test_size: (frac or Int) and number:(True or False) should be set correctly")
...
def time_series_cv(n, n_folds, test_size=.2):
    ...
    '''
    out = []
    split_points = [(n * i / float(n_folds), n * (i + 1) /
                     float(n_folds)) for i in range(n_folds)]
...
def cascade_cv(n, n_folds, data_size=.8, test_size=.15, number=False):
    ...
    if shift < 4:
        raise (UserWarning(
            "Small Shift warning: Consider less folds, or a smaller data size"))
